...
import { normalizzaProposte, citazioniGemini, citazioniClaude, distanzaGeograficaKm, chiaveUrl, verificaNomiProposte, nomePresenteNellaFonte } from './proposte-scout.js'
...
test('configurazione iniziale: include anche i luoghi vicini senza cambiare le fasce manuali', () => {
  const fonte = { ...candidato.fonti[0], campi: ['nome', 'descrizione', 'distanza_km'] }
  const vicino = { ...candidato, distanza_km: 0.5, fonti: [fonte] }
  assert.equal(normalizzaProposte([vicino], [url], [], 5).length, 0)
  const [proposta] = normalizzaProposte([vicino], [url], [], 5, true)
  assert.equal(proposta.verifica.fascia_min_km, 0)
  assert.equal(proposta.verifica.fascia_max_km, 5)
  assert.equal(normalizzaProposte([{ ...vicino, distanza_km: 6 }], [url], [], 5, true).length, 0)
  assert.equal(normalizzaProposte([{ ...vicino, fonti: candidato.fonti }], [url], [], 5, true).length, 0)
})
...
test('il nome inventato Vaillum non passa con una fonte che riporta Vatillum', () => {
  const fonte = { url, titolo: 'Vatillum Pizzeria Paestum | Menu' }
  const corretta = { ...candidato, nome: 'Vatillum Pizzeria Paestum' }
  const refuso = { ...corretta, nome: 'Vaillum Pizzeria Paestum' }
  assert.deepEqual(verificaNomiProposte([refuso, corretta], [fonte]), [corretta])
  assert.equal(verificaNomiProposte([{ ...refuso, fonti: [{ ...candidato.fonti[0], titolo: refuso.nome, conferma: refuso.nome }] }], [fonte]).length, 0)
})

test('verifica il nome nella fonte realmente citata, senza usare un’altra pagina', () => {
  assert.equal(verificaNomiProposte([candidato], [{ url: 'https://altro.it', titolo: candidato.nome }]).length, 0)
  assert.equal(verificaNomiProposte([candidato], [{ url }]).length, 0)
  assert.equal(verificaNomiProposte([candidato], [{ url, nome: candidato.nome }]).length, 1)
  assert.equal(verificaNomiProposte([candidato], [{ url, testo: `Il menu della ${candidato.nome} propone pizze.` }]).length, 1)
  assert.deepEqual(verificaNomiProposte([null, {}], []), [])
})

test('accetta maiuscole e punteggiatura, ma non somiglianze o parti di parole', () => {
  assert.ok(nomePresenteNellaFonte('Pizzeria Sant’Angelo', { titolo: "PIZZERIA SANT'ANGELO — sito ufficiale" }))
  assert.equal(nomePresenteNellaFonte('Vaillum', { titolo: 'Vatillum' }), false)
  assert.equal(nomePresenteNellaFonte('Roma', { titolo: 'Romano' }), false)
  assert.equal(nomePresenteNellaFonte('', { titolo: 'Vatillum' }), false)
})

test('il percorso Geoapify scarta Vaillum anche se la fonte cartografica contiene il refuso', () => {
  const proposta = { ...candidato, nome: 'Vaillum' }
  assert.equal(normalizzaProposte([proposta], [url], ['Vatillum Pizzeria Paestum']).length, 0)
  assert.equal(normalizzaProposte([{ ...candidato, nome: 'Las Vegas' }], [url], ['Vatillum Pizzeria Paestum']).length, 1)
})
