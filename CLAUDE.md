...
- **Struttura di prova «Villa Demo (dati di esempio)» (26/09/2026)** — `supabase/dati-esempio/villa-demo.sql` (+ `villa-demo-rimuovi.sql`), **non sono migration**: script di DATI da lanciare a mano nel SQL Editor. **Perché**: nessun host ha soggiorni veri per misurare l'adozione (Villa Virginia è stagionale: ospiti giugno-agosto, nessuna prenotazione ora, accoglienza sempre di persona), e l'utente ha chiesto dati «realistici, plausibili». **Decisione (mia, spiegata all'utente): NON inserire dati inventati in Villa Virginia** — ha ospiti veri, il cruscotto serve a mostrare l'uso REALE, e in estate le aperture non venivano nemmeno contate (la 0024 è del 24/09), quindi un dato vero non esiste; numeri di fantasia mescolati ai veri renderebbero indistinguibile il reale dal simulato e rischierebbero di finire davanti a un cliente come risultato vero. Si crea invece una struttura SEPARATA `villademo` (nome «Villa Demo (dati di esempio)», `attivo=false` = non pubblica, 4 unità, del superadmin): 14 soggiorni con date relative a OGGI (12 arrivati negli ultimi 30 giorni + 2 in arrivo), 8 su 12 con guida aperta (**67%**), **3,0** aperture in media, **1 ospite in casa senza guida** (fa comparire l'avviso ambra), 85 domande a Gennarino in 30 giorni. Idempotente (`delete` della struttura e ricreazione; soggiorni e statistiche seguono per `on delete cascade`), rilanciabile per «rinfrescare» le date; la rimozione filtra SOLO `slug='villademo'`. Verificato su PostgreSQL in memoria con le migration vere 0021/0023/0024 (20 controlli: numeri del cruscotto calcolati con `lib/adozione.js`, token unici, date coerenti, nessun dato orfano, dati veri di un'altra struttura intatti prima/dopo/rimozione). ⚠️ **Sono dati di prova: non presentarli mai come risultati reali** (dirlo se si mostrano schermate). Compare nel selettore struttura del superadmin. Da questa sessione non si può scrivere nel database di produzione (la `SUPABASE_SERVICE_ROLE_KEY` è solo su Vercel): li lancia l'utente.
