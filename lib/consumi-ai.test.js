...
test('classifica gli errori senza conservarne il testo', () => {
  assert.equal(tipoErroreAI(new Error('RESOURCE_EXHAUSTED quota')), 'credito o limite')
  assert.equal(tipoErroreAI(new Error('HTTP 402 Payment required')), 'credito o limite')
  assert.equal(tipoErroreAI(new Error('HTTP 403 API key')), 'chiave o permessi')
  assert.equal(tipoErroreAI(new Error('HTTP 400 invalid_request_error')), 'richiesta non valida')
  assert.equal(tipoErroreAI(new Error('HTTP 503 overloaded')), 'servizio non disponibile')
