...
export function promptScout({ struttura, categoria, daEscludere, raggioKm, raggioCompleto = false }) {
  const minimo = raggioCompleto ? 0 : minimoFascia(raggioKm)
...
export function normalizzaProposte(candidati, citazioni, esclusi = [], raggioKm, raggioCompleto = false) {
  ...
  for (const c of candidati) {
    ...
    if (!nome || !indirizzo || !descrizione || [...descrizione].length > 200 || possibileDuplicato(nome, [...nomi])) continue
    ...
    if (raggioKm != null && (!supportato('distanza_km') || typeof c.distanza_km !== 'number'
      ...
      || (minimo > 0 && c.distanza_km <= minimo))) continue
    ...
    proposta.verifica = { indirizzo, fonti, distanza_km: c.distanza_km ?? null, fascia_min_km: minimo, fascia_max_km: raggioKm ?? null, non_verificato: nonVerificato,
