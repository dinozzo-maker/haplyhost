...
- `soggiorni`: dalla migration 0013 non ha più NESSUNA policy pubblica (quella per-soggiorno-attivo è stata rimossa senza sostituzione) — non esiste un modello di accesso sicuro lato ospite. Non blocca nulla oggi (il frontend non la usa ancora, vedi "Non ancora iniziato"), ma va deciso prima di costruirci sopra il Wi-Fi-per-soggiorno.
...
- Wi-Fi legato al soggiorno attivo (tabelle `strutture_segreti` e `soggiorni` pronte, nessuna UI/logica costruita) —
  **diverso** dalle "reti Wi-Fi multiple" (17-22/09/2026, vedi sopra): quella è la lista statica delle reti della
  casa, editabile dall'host; questa sarebbe mostrare all'ospite giusto la rete/nota valida per il SUO soggiorno.
